...
const fetchData = async () => {
    ...
    try {
        ...
        const res = await api.get(`/job-seeker-educations/${profileId.value}`);
        if (res.data.status) {
            profile.value = res.data.data.profile;
            educations.value = res.data.data.educations;
        }
    } catch (e) {
        toast.error("Academic profile not found");
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="text-left">
                    ...
                    <p v-if="profile" class="text-sm text-gray-500 flex items-center gap-2 mt-0.5">
                        ...
                        <template v-else>
                            Manage your verified qualifications and scholarly records.
                        </template>
                    </p>
                </div>
            </div>
        ...
        <div class="relative">
            ...
            <div v-else class="space-y-4">
                <div v-for="edu in educations" :key="edu.id" 
                    class="group bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-all animate-in slide-in-from-bottom-2 text-left">
                        ...
                        <div class="flex flex-col items-end gap-2 min-w-[160px] md:w-1/3">
                            <div class="flex items-center gap-3 h-9">
                                ...
                                <div class="flex items-center gap-2 opacity-0 translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-500 order-2">
                                    <button @click="openEditModal(edu)" class="h-9 w-9 flex items-center justify-center text-gray-400 hover:text-indigo-600 bg-gray-50 rounded-xl transition-all shadow-sm">
                                        <i class="fa-solid fa-pencil text-sm"></i>
                                    </button>
                                    <button @click="confirmDelete(edu.id)" class="h-9 w-9 flex items-center justify-center text-gray-400 hover:text-red-500 bg-gray-50 rounded-xl transition-all shadow-sm">
                            ...
                            <div class="flex flex-col items-end gap-1">
                                <div :class="getLevelClass(edu.education_level)" 
                                    class="px-2 py-0.5 rounded-lg border text-[10px] font-black tracking-tighter uppercase">
                                    {{ edu.education_level }}
                                </div>
                    ...
                    <div v-if="edu.field_of_study || edu.description" class="mt-4 pt-4 border-t border-gray-50 space-y-2">
